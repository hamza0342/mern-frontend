...
import MainLayout from "../Client/client-components/Layout";
import GC_Pop_up_Model_Mir from "./GC_Pop_up/GC_Pop_up_Model_Mir"
...
import { getmirFilebyFolder, getPdfmirFilebyFolder, getApprovedFiles } from "../Client/clientapi";
import GC_Pop_up_Pdf from "./GC_Pop_up/GC_Pop_up_Pdf";
...
const subContractor = isAuthenticated().subContractor.name
...
let handleChildUpdate = (req,res)=>{
  ...
  useEffect(() => {
    ...
    getmirFilebyFolder(token, folderId).then((response) => {
    ...
    getPdfmirFilebyFolder(token).then((response) => {
    ...
    getApprovedFiles(token).then((response) => {
  ...
  return (
    <>
      ...
      <div className="upload_files">
        <GC_Pop_up_Model_Mir getfile={getfile} callback={handleChildUpdate} />
                ...
                <tr className="rowsss">
                  ...
                  <td style={{ width: 500 }}>
                        ...
                        iss.map((iss) => (
                          <div style={{ backgroundColor: "white", border: "2px  gray", borderBottom: "none", borderLeft: "none", borderRight: "none" }}>
...

                            {
                              file._id == iss.belongsTo ?
                                (
                                  <div style={{ float: "right",  marginTop:-2, cursor: "pointer" }}>
                                    {
                                      subContractor == iss.uploadedBy ? (<DeleteIcon style={{ color: "red" }} onClick={() => deleteSub(iss._id)} />): (" ")
                                    }
                            ...
                            }
                            {
                              file._id == iss.belongsTo ?
                                (
                                  <div style={{ float: "right", marginTop: -2, cursor: "pointer", marginRight: 10 }}>
                              ...
                              file._id == iss.belongsTo ?
                                (
                                  <div style={{ marginBottom: 5, paddingLeft: 5 }}>
                                    ...
                                    <div>
                                    <p>Uploaded By : {subContractor == iss.uploadedBy ? (" You") : (iss.uploadedBy)} </p>
                                      <h6
                                        style={{
                                          float: "right",
                                          marginTop: -105,
                                      ...
                                      >
                                        {/* {
                                          Client == iss.uploadedBy ?
                                            (" ") : (<button className="resolve" onClick={() => idesssss(iss._id, file._id)} >Approved</button>)
                                        } */}
                                      </h6>
                                    </div>
                    ...
                    <div style={{ textAlign: "center" }}>
                      ...
                      <GC_Pop_up_Pdf fileId={file._id} idHandler={FileId}  callbacks={handleChildUpdate}/>
                    </div>
                  </td>
